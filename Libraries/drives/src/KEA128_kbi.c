...
//								NVIC_SetPriority(KBI1_IRQn,1);					//设置优先级,根据自己的需求设置 可设置范围为 0 - 3
//								NVIC_EnableIRQ(KBI1_IRQn);							//打开KBI1_IRQn的中断开关
...
void kbi_init(KBI_CHn chn, TRIG_CFG cfg)
{
    ...
    //屏蔽KBI中断
    kbi[kbi_x]->SC &= ~(uint32_t)KBI_SC_KBIE_MASK;

    //设置触发模式
    if((IRQ_FALLING == cfg) || (IRQ_FALLING_LOW == cfg))
        kbi[kbi_x]->ES &= ~((uint32_t)1<<kbi_n);
    else
        kbi[kbi_x]->ES |= ((uint32_t)1<<kbi_n);
    
    //设置上拉
    port_pull((PTX_n)((uint8_t)(chn)));
