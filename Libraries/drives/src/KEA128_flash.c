...
//  @return     				返回一个扇区的大小
//  @since      v1.0
//  Sample usage:               uint32_t dat = FLASH_GetSectorSize();
//-------------------------------------------------------------------------------------------------------------------
...
//  @since      v1.0
//  Sample usage:               uint32_t dat = FLASH_GetSectorSize(10);
//-------------------------------------------------------------------------------------------------------------------
...
//  @param      *buf	 		数据首地址
//  @param      len		 		写入的字节数 特别提醒一个uint16_t变量是两个字节 一个uint32_t变量是四个字节 每次写入得长度必须为4得倍数
//  @param      offset		 	必须为4的整数倍
//  @return     				返回1写入失败，返回0写入成功
//  @since      v1.0
//  Sample usage:               FLASH_WriteSector(10,(const uint8_t *)buf,4,0);//将buf数组里面的前四位数据写入到10号扇区(每一位是uint8_t类型)
