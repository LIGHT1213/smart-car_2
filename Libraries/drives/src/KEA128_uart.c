...
uint32_t UART_Init(UARTn uartn, uint32_t baud)
{
  ...
  (SIM->PINSEL1 |=  SIM_PINSEL1_UART2PS_MASK) ;
    ...
    uart[uartn]->BDL  = UART_BDL_SBR((uint8_t)sbr);               //再写入SBR低位
...
void uart_getchar (UARTn uartn, uint8_t *ch) 
{
    ...
    *ch =  uart[uartn]->D;                          // 获取接收到的8位数据
    // 获取 9位数据时ch 应该是uint16_t *类型的否则数据会溢出：
...
void uart_rx_irq_en(UARTn uartn)
{
    ...
    NVIC_EnableIRQ((IRQx_Type)((uint8_t)uartn + UART0_IRQn));                                //使能IRQ中断
}
...
void uart_rx_irq_dis(UARTn uartn)
{
    ...
    {
        NVIC_DisableIRQ((IRQx_Type)((uint8_t)uartn + UART0_IRQn));                           //关IRQ中断
    }
}
