...
{
    uint16_t num;
    uint16_t wait_num=500;//等待次数，超过设定的次数将关闭硬件IIC，使用模拟IIC发出stop信号，然后在启动硬件IIC。
    ...
    while(( I2CN[i2cn]->S & I2C_S_IICIF_MASK)==0)
    {
        ...
        {
            I2CN[i2cn]->C1 = 0;
            if(i2cn == i2c0)    SIM->SCGC &= ~(uint32_t)SIM_SCGC_I2C0_MASK;         
            else                SIM->SCGC &= ~(uint32_t)SIM_SCGC_I2C1_MASK;           
            IIC_init();
            IIC_stop();
...
//-------------------------------------------------------------------------------------------------------------------
uint32_t i2c_init(I2Cn_e i2cn, uint32_t baud)
{
    //查表 ICR 对应的  SCL_divider ，见 《KEA128RM.pdf》第537页的I2C 分频器和保持值
    uint16_t ICR_2_SCL_divider[0x40]  =
    {
    ...
    uint16_t scldiv =  0;  //最佳的分频系数

    //需要从 ICR_2_SCL_divider 里找到 与最佳分频系数scldiv最相近的 分频系数
    uint8_t icr, n;
    uint16_t min_Dvalue = ~0, Dvalue;
    ...
    else
    {
        ...
        if(E1 == I2C1_SCL_PIN)
        {
            SIM->PINSEL1 &= ~(uint32_t)SIM_PINSEL1_I2C1PS_MASK;
        }
        else
        {
    ...
    mult = CLK_BUS/baud/3840;

    scldiv =  CLK_BUS / ( (1<<mult) * baud );    //最佳的分频系数
    ...
    return (  CLK_BUS/ ( (1<<mult) * ICR_2_SCL_divider[icr])  );
}
...
void i2c_delay(void)
{
    volatile uint16_t n = 10;     //注意，这个数据太小，会导致读取错误。
...
//  @return                 读取的寄存器值
//  @since      v2.0
//  Sample usage:       	uint8_t value = i2c_read_reg(i2c0, 0x2D, 0x50);//读取0x50地址的数据，从机地址为0x2D
//-------------------------------------------------------------------------------------------------------------------
...
//  @return     void
//  @since      v2.0
//  Sample usage:       	uint8_t value = i2c_read_reg(i2c0, 0x2D, 0x50, 10, buf);//读取0x50地址的数据，从机地址为0x2D开始的10个字节
//-------------------------------------------------------------------------------------------------------------------

uint8_t i2c_read_reg_bytes(I2Cn_e i2cn, uint8_t SlaveID, uint8_t reg, uint8_t num, uint8_t * addr)
{
    //先写入寄存器地址,再读取数据,因此此过程是 I2C 的复合格式,改变数据方向时需要重新启动
    //地址是低七位
    uint8_t result;
