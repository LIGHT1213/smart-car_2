...
//  @return     				返回真实波特率
//  @since      v2.0
//  Sample usage:       		uint32_t baud = spi_init(spi0,USE_PCS,MASTER,1*1000*1000);              //初始化SPI,主机模式, 波特率为1M ,返回真实波特率到baud变量
//-------------------------------------------------------------------------------------------------------------------
uint32_t spi_init(SPIn_e spin, SPI_PCSn_e pcs, SPI_CFG master,uint32_t baud)
{
    uint8_t i,j;
    uint8_t sppr,spr;
    uint32_t Scaler[] = {2,4,8,16,32,64,128,256,512};
    uint8_t  Prescaler[] = {1,2,3,4,5,6,7,8};
    ...
    else
    {
        ...
        if(D0 == SPI1_SCK_PIN)
        {
            SIM->PINSEL1 &= ~(uint32_t)SIM_PINSEL1_SPI1PS_MASK;
        }
        else
        {
    ...
    if(master == MASTER)
    {
        ...
        for(i=0; i<9; i++)
        {
            for(j=0; j<8; j++)
            {
                diff = abs(CLK_BUS/(Prescaler[j]*Scaler[i]) - baud);
                if(min_diff > diff)
                {
                    //更新最佳参数
                    sppr = j;
    ...
    return (CLK_BUS/Prescaler[sppr]/Scaler[spr]);      //返回实际波特率
}
