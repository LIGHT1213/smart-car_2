...
{	    
    ...
    wx = (uint16_t)dat[0]<<8 | dat[1];
    //补码求出加速度原始对应数值
    if(dat[0]>0x7f) acc_x=-(int16_t)((~(wx>>2) + 1)&0X3FFF);  //移位取反加一再去掉无效字符 
    else            acc_x=(wx>>2)&0X3FFF; 	                //移位掉无效字符 	

    
    wy = (uint16_t)dat[2]<<8 | dat[3];
    //补码求出加速度原始对应数值
    if(dat[2]>0x7f) acc_y=-(int16_t)((~(wy>>2) + 1)&0X3FFF);  //移位取反加一再去掉无效字符 
    else            acc_y=(wy>>2)&0X3FFF; 	                //移位掉无效字符 	

    
    wz = (uint16_t)dat[4]<<8 | dat[5];
    //补码求出加速度原始对应数值
    if(dat[4]>0x7f) acc_z=-(int16_t)((~(wz>>2) + 1)&0X3FFF);  //移位取反加一再去掉无效字符 
