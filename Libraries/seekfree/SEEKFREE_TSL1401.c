...
uint16_t ccd_data1[128];                           //CCD1数据
uint16_t ccd_data2[128];                           //CCD2数据
...
{       
    ...
    DisableInterrupts;
    pit_init_ms(ZERO, EXPOSURE_TIME);   		//定时EXPOSURE_TIME(ms) 后中断
    NVIC_SetPriority(PIT_CH0_IRQn,1);			//设置优先级,根据自己的需求设置 可设置范围为 0 - 3  越小优先级越高
